                    ...
                    <React.Fragment key={index}>
                        ...
                        >
                            <div className='overflow-hidden w-24 h-24' onClick={()=>handleClick()}>
                                <LazyLoadImage
                                    src={`${ASSETS_URL.ezo}${JSON.parse(hot.images)[0].name.replace('products/', '')}`}
                                    alt={`${hot.title}`}
                                    width={96}
                                    height={96}
                                    effect='blur'
                            ...
                            <div className='flex flex-col gap-2 w-56' onClick={()=>handleClick()}>
                                ...
                                <label>
                                    {hot?.usdLow !== hot.usdHigh &&
                                        <span className='line-through text-sm'>
                                            {strPrice(hot?.usdLow * currencyRate.time, currencyRate.currency)}
                                        </span>
                                    }
                                    <span className='text-accent ml-2'> {strPrice(hot?.usdLow * currencyRate.time, currencyRate.currency)}</span>
                                </label>
                            </div>
                        </Link>
                    </React.Fragment>
...
export default function SearchHeader() {
    ...
    const handleExtendMenu = (category) => {
        const subItems = categories.filter((c) => (c.parentId === category.id && c.id !== category.id));
                        ...
                        items={
                            mainCategories.map((category, index) => (
                                ...
                                >
                                    ...
                                    {
                                        categories.filter((c) => (c.parentId === category.id && c.id !== category.id)).length > 0 &&
