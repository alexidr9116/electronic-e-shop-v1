...
export default function MobileNavDrawer({ open, onClose, routers = [], categories = [] }) {
    ...
    return (
        <Drawer className="min-w-[320px]" open={open} onClose={onClose} children={<>
            ...
            <div className="flex flex-col w-full px-2 ">
                {/* shop by categories */}
                {
                    currentTab === 1 && categories.map((category, index) => (
                        <Link key={index} className={`border-b ${themeMode === 'light' ? 'border-stone-300' : 'border-gray-800'} p-3`} to={`products/shop-by-category/${category.key}`}>
                            {category.value}
                        </Link>
                    ))
                }
            ...
            <div className={`absolute w-full max-w-[320px] justify-end gap-2 flex bottom-0 border-t ${themeMode === 'light' ? 'border-stone-300' : 'border-gray-800'} p-2`}>
                {SUPPORT_LOCALES.map((locale, index) => (
                    <button className={`btn btn-sm border-0 bg-none p-0  btn-ghost justify-start ${language.key === locale.key ? 'btn-active':''}`} key={locale.key}>
