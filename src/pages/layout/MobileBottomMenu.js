...
export default function MobileBottomMenu() {
    ...
    return (
        <div className="w-full grid grid-cols-5 bg-white fixed bottom-0 left-0 border-t items-center text-xs">
            ...
            <div className={` flex flex-col w-full py-1 text-center ${currentTab === 3 ? 'border-b-2  border-red-500' : 'border-b'}`} onClick={() => setCurrentTab(3)}>
                <div className="flex justify-center"><Icon icon ='ic:sharp-favorite-border' width = {20}/></div>
                <label>{t('words.favorite')}</label>
            </div>
            <div className={` flex flex-col w-full py-1 text-center ${currentTab === 4 ? 'border-b-2  border-red-500' : 'border-b'}`} onClick={() => setCurrentTab(4)}>
