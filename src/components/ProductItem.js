        ...
        <div className={`border ${borderColor} rounded-lg hover:border-accent flex   ${layout === 'grid' ? 'flex-col' : 'flex-col sm:flex-row'} w-full  p-2 relative cursor-pointer `} >
            <div className={`flex gap-2 justify-center`}>
                ...
                <LazyLoadImage alt={`lazy-image`}
                    ...
                    wrapperClassName={`overflow-hidden ${layout === 'list' ? 'w-[150px] lg:w-[250px] h-[150px]' : 'w-[300px] h-[300px]'}  ${product.stockAmount <= 0 ? 'opacity-40' : ''}`}
            ...
            <div className={`flex w-full flex-col justify-center p-2 ${layout === 'grid' ? 'p-2' : 'p-4'}`}>
                <TextMaxLine maxLine={1} text={product.title} className='font-bold' onClick={handleClick} ></TextMaxLine>
                {(layout === 'list') && <TextMaxLine text={product.description} className='hidden md:flex-1 md:mt-2' />
                }
                <div className='mt-1'>
                ...
                <div className='flex gap-2 items-center'>
                    {(product.usdLow > 0 && product.usdLow !== product.usdHigh) &&
                        <label className='line-through '>{strPrice(product.usdHigh * currencyRate.time, currencyRate.currency)}</label>
                    }
                    <label className='text-accent font-bold text-lg'>{strPrice(product.usdLow * currencyRate.time, currencyRate.currency)}</label>
                </div>
                {(layout === 'list') && <div className='flex mt-2 gap-2'>
                    ...
                    }} disabled ={product.stockAmount<=0}><Icon icon='carbon:shopping-cart-arrow-down' width={20}></Icon>{t('shopping.add-cart')}</button>
